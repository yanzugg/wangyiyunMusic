...
>
 <p class="hot_title">最佳匹配</p>
       <!-- <van-cell center :title="obj.name"  :label="obj.ar[0].name+ ' - ' +obj.name" v-for="obj in resultList" :key="obj.id">
       <template #right-icon>
    <van-icon name="play-circle-o" size="0.7rem"/>
  </template>
    </van-cell> -->
   <SongItem v-for="obj in resultList" :key="obj.id" :name="obj.name" :authour="obj.ar[0].name" :id="obj.id"></SongItem>
</van-list> 
      </div>
    </div>
</template>
...
// 2.找接口，api/Search.js里定义获取搜索关键词的请求
// 3.引入到当前页面，调用接口拿到数据循环铺设页面
// 4.点击关键词，把值赋予给van-search的v-model变量


// 目标：铺设搜索结果
// 1.找到搜索结果的接口 - api/Search.js定义请求方法
// 2.再定义methods里getListFn(获取数据)
...
// 目标：搜索框-显示结果
// 1.@input给van-serach绑定事件
// 2.@input事件中，获取搜索结果保存resultList中
// 3.判断如果搜索关键词为空(.length===0),数组清空，阻止网络请求执行(提前return)

// 目标：加载更多
// 1.集成list组件-定义相关的变量(搞懂变量的作用)
// 2.一旦触底，自动执行onload方法
...
data() {
  ...
  methods: {
    ...
    // 效果：用户在n秒内不触发这个事件，才会执行这个逻辑代码
     if(this.timer) clearTimeout(this.timer)
     this.timer = setTimeout(async()=>{
       this.page=1 //点击重新获取第一页数据
      this.finished = false //输入框改变可能有新的更多数据
      if(this.value.length===0){ //搜索关键词如果没有，就把搜索结果清空阻止网络请求发送(提前return)
        this.resultList=[]
        return 
      }
      const res = await this.getListFn() 
    ...
    async onLoad(){  //触底事件(要加载下一页的数据)，内部会自动把loading改为true
          this.page++
          const res = await this.getListFn()
         if(res.data.result.songs===undefined){  //没有更多数据
          this.finished = true //只要设置true，list再次触发都不会执行onload函数(显示"没有更多数据")
           this.loading = false
         return
        }
